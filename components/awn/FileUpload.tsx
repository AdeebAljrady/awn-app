...
//          type: "application/pdf",
  ...
  const handleFile = (file: File | undefined) => {
    ...
    // Validate type - only PDF as per current requirement
...
if (!ALLOWED_FILE_TYPES.includes(file.type)) {
  toast.error("يرجى رفع ملف PDF أو Word أو PowerPoint", {
    position: "bottom-center",
    duration: 3000,
  });
  return;
...
//    return <FileIcon className="h-5 w-5 text-foreground" />;
 if (!uploadState.file) return <FileIcon />;
  ...
  const type = uploadState.file.type;

  if (type.includes("pdf")) return <FileIcon className="h-5 w-5 text-red-600" />;
  if (type.includes("word")) return <Scroll className="h-5 w-5 text-blue-600" />;
  if (type.includes("presentation")) return <FileSpreadsheet className="h-5 w-5 text-orange-600" />;
